...
export const USDC = {
  ...
  [ChainId.TESTNET]: new Token(ChainId.TESTNET, TESTNET.mockUSDC, 18, 'USDC', 'Mock USDC'),
}
...
export const DIFFUSION = makeToken('PepeSwap.lol Token', 'CROAK', 18, {
  [ChainId.MAINNET]: MAINNET.diffusion,
  [ChainId.TESTNET]: TESTNET.diffusion || '0x067eC87844fBD73eDa4a1059F30039584586e09d',
})

export const XDIFFUSION = makeToken('xCROAK', 'XCROAK', 18, {
...
function makeToken(name: string, symbol: string, decimals: number, addresses: Record<ChainId, string>) {
  return {
    ...
    [ChainId.TESTNET]: new Token(ChainId.TESTNET, addresses[ChainId.TESTNET], decimals, `${symbol}`, `Mock ${name}`),
  }
}
